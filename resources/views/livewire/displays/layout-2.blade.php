    <div class="absolute inset-0 z-0 transition-opacity duration-500">
        <div class="w-full h-full"
             :style="'background: radial-gradient(ellipse at top right, var(--theme-light), #f8fafc, #f1f5f9); opacity: 0.3;'">
        </div>
    ...
    <header x-show="mode === 'standby'"
            ...
            <div class="marquee-preview w-full">
                <div class="marquee-content font-bold uppercase tracking-wide leading-tight text-[3.5vh]"
                    ...
                    <div class="marquee-item">
                        ...
                        <span class="text-slate-600">/ {{ $jadwal->tanggal_hijriah ?? '' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end col-span-4">
            <div class="text-slate-900 text-[7vh] font-black tracking-widest leading-none drop-shadow-2xl tabular-nums transition-colors duration-500"
    ...
    <div class="w-full max-w-[1080px] min-w-[640px] flex flex-col gap-[2vh]">
        ...
        <div class="relative w-full z-20 shrink-0">
            <div
                class="bg-white/90 border-slate-800 shadow-xl ring-slate-700 h-[8vh] backdrop-blur-2xl border rounded-[4vh] flex items-center overflow-hidden ring-1 transition-colors duration-500">
                <div
                    class="border-slate-200 shadow-[10px_0_30px_rgba(0,0,0,0.1)] bg-theme-main h-full flex items-center px-[2vw] z-30 border-r relative transition-colors duration-500">
                ...
                <div class="marquee-preview flex-1">
                    <div class="marquee-content font-black text-[3.5vh] uppercase tracking-[0.05em]"
                        ...
                        @empty
                            <div class="text-slate-600 marquee-item transition-colors duration-500">
                                <svg class="text-slate-400 w-[4vh] h-[4vh] animate-spin-slow shrink-0 transition-colors duration-500" viewBox="0 0 24 24"
                                    fill="currentColor">
                                    <path
                                        d="M12,2L14.47,4.53L17.94,3.53L18.94,7.06L22.47,8.53L21.47,12L22.47,15.47L18.94,16.94L17.94,20.47L14.47,19.47L12,22L9.53,19.47L6.06,20.47L5.06,16.94L1.53,15.47L2.53,12L1.53,8.53L5.06,7.06L6.06,3.53L9.53,4.53L12,2Z" />
                                </svg>
    ...
    <div class="flex-1 flex flex-col min-h-0 rounded-[2vh] border-4 transition-colors duration-500 overflow-hidden border-slate-300 shadow-2xl">
    ...
    <div x-show="mode === 'menuju_adzan'" style="display: none;"
        class="bg-white/95 bg-frame absolute inset-0 z-50 flex flex-col items-center justify-center backdrop-blur-3xl transition-colors duration-500">
        <h2 class="text-slate-600 text-[6vh] md:text-[8vh] text-center font-bold uppercase tracking-[0.5em] mb-[1vh] transition-colors duration-500">Menjelang</h2>
        <p class="text-theme-dark text-[6vh] md:text-[8vh] mb-[3vh] tracking-widest uppercase font-black animate-pulse transition-colors duration-500">Adzan <span
                x-text="currentPrayerName" class="text-slate-900 transition-colors duration-500"></span></p>
        <h1 class="text-slate-900 shadow-none text-[25vh] font-black leading-none tabular-nums transition-colors duration-500"
            x-text="countdownAdzanDisplay">00:00</h1>
    </div>
    <div x-show="mode === 'adzan'" style="display: none;"
        class="bg-white/95 bg-frame fixed inset-0 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl transition-colors duration-500">
        <h2 class="text-slate-600 text-[6vh] md:text-[8vh] text-center font-bold uppercase tracking-[0.5em] mb-[3vh] transition-colors duration-500">Adzan <span
                x-text="currentPrayerName" class="text-slate-900 transition-colors duration-500"></span></h2>
        <p class="text-theme-dark text-[6vh] md:text-[8vh] tracking-widest uppercase font-black animate-pulse transition-colors duration-500">Telah Masuk</p>
    </div>
    <div x-show="mode === 'waiting_iqomah'" style="display: none;"
        class="bg-white/95 bg-frame fixed inset-0 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl transition-colors duration-500">
        <h2 class="text-theme-dark text-[6vh] mb-[4vh] tracking-[0.5em] text-center uppercase font-bold transition-colors duration-500">Menuju Iqomah:</h2>
        <h1 class="text-slate-900 shadow-none text-[20vh] mb-[4vh] font-black leading-none tabular-nums transition-colors duration-500"
            ...
            <template x-if="iqomahContents.length > 0">
                <template x-for="(content, index) in iqomahContents" :key="index">
                    <p x-show="activeIqomahContentIndex === index"
                       ...
                       class="text-slate-900 text-[3vh] md:text-[4vh] px-[10vw] text-center uppercase font-black transition-colors duration-500 absolute w-full"
                       x-text="content.teks"></p>
                </template>
            </template>
            <template x-if="iqomahContents.length === 0">
                <p class="text-slate-900 text-[3vh] md:text-[4vh] text-center uppercase tracking-wider font-black transition-colors duration-500 w-full animate-pulse">LURUSKAN DAN RAPATKAN SHAF</p>
            </template>
        </div>
    </div>
